...
st.markdown('It is important to mention that any police district can respond to any incident, and the neighborhood in which it happened is not related to the police district.')

st.markdown('Crime locations in San Francisco')
st.map(subset_data)

df1 = df.groupby(['Incident Year']).size().reset_index(name='Count')
graf_year = alt.Chart(df1, width=300, height=300).mark_bar(
    color='#2A3132',
    height=40
).encode(
    y=alt.Y('Incident Year:N'),
    x=alt.X('Count:Q')
).properties(title='Crimes occurred per year')

col1, col2 = st.columns(2)
col1.altair_chart(graf_year, use_container_width=True)
...
with col3:
    ...
    st.bar_chart(df['Report Type Description'].value_counts())

with col4:
    st.markdown('Resolution status')
    fig1, ax1 = plt.subplots()
    labels = subset_data['Resolution'].unique()
    ax1.pie(subset_data['Resolution'].value_counts(), labels=labels, autopct='%1.1f%%', startangle=20, colors=['#c63637', '#e2504c', '#ff6961', '#ff9688'])
    st.pyplot(fig1)